...
type TufSignedContent interface {
	...
	Exists(ctx context.Context, repoID data.RepoID, ver int) (bool, error)
	...
	FindVersion(ctx context.Context, repoID data.RepoID, ver int) (*data.SignedRootRole, error)
	// GetMaxVersion returns current repo version
	GetMaxVersion(ctx context.Context, repoID data.RepoID) (int, error)
}
