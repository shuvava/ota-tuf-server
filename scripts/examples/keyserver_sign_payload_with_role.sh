...
ALLOWED_ROLES=("ROOT" "SNAPSHOT" "TARGETS" "TIMESTAMP")
DEFAULT_ROLE="ROOT"
...
-r, --repo      TUF repo UUID
--role          TUF repo role ${DEFAULT_ROLE}
EOF
  exit
}
...
parse_params() {
  ...
  [[ "${ALLOWED_ROLES[*]}" =~ (^|[[:space:]])"$ROLE"($|[[:space:]]) ]] || die "parameter ${YELLOW}role${NOFORMAT} has incorrect value"
...
URL="${TUF_REPO_URL}/api/v1/root/${REPO_ID}/${ROLE}"
# example of the body to sign
BODY="{\"keyType\":\"${ROLE}\", \"threshold\": 2}"

msg "${GREEN}RequestID:${NOFORMAT} ${REQUEST_ID}"
msg "${GREEN}RepoID   :${NOFORMAT} ${REPO_ID}"
msg "${GREEN}ROLE     :${NOFORMAT} ${ROLE}"
